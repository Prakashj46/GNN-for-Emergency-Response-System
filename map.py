...
num_injured = st.number_input("Enter number of injured people: ", min_value=0)  # Simulating injured people
...
# Check if any hospital has sufficient capacity
available_hospitals = hospital_df[hospital_df['Beds Available'] >= num_injured]
if not available_hospitals.empty:
    # Find the nearest available hospital
    nearest_hospital_idx = available_hospitals['Availability Prediction'].idxmax()
    nearest_hospital = available_hospitals.loc[nearest_hospital_idx]
    ...
    st.title("Nearest Available Hospital")
    st.markdown(f"Accident Location:\nLatitude: {accident_location['Latitude']}\nLongitude: {accident_location['Longitude']}")
    st.markdown(f"Number of Injured: {num_injured}")
    st.markdown("Nearest Available Hospital:")
    st.write(nearest_hospital)

    st.markdown("Navigation Route:")
    st.write(navigation_route)

else:
   st.warning(f"No hospital has enough available beds to accommodate {num_injured} injured people.")

st.subheader('Hospital Map:')
st.title("Map Display")

with open("mod_html.html", "r") as f:
   map_html = f.read()

# Display the map HTML using components.iframe()
st.components.v1.html(map_html, width=1000, height=600)

# Save the model
hospital_map.save('mod_html.html')